...
export async function startRedis(): Promise<void> {
	if (privateRedisClient && privateSubscriberClient) {
		log.info("Redis clients already initialized.");
		return;
	}
	...
	privateRedisClient = createClient({
		RESP: 3,
		password:
			DMNO_CONFIG.APP_ENV === "production"
				? DMNO_CONFIG.REDIS_APP_PASSWORD
				: undefined,
		socket: {
			host: DMNO_CONFIG.REDIS_APP_HOST,
			port: DMNO_CONFIG.REDIS_APP_PORT,
			connectTimeout: 5000,
	...
	const attachListeners = (
		...
		client.on("connect", () => log.info(`${name} connected to Redis`));
		client.on("error", (err) => console.error(`${name} Redis error:`, err));
		client.on("reconnecting", () =>
			log.info(`${name} reconnecting to Redis...`),
		);
	};
	...
	try {
		...
		log.info("✅ Redis clients connected successfully");
...
export async function stopRedis(): Promise<void> {
	if (!privateRedisClient || !privateSubscriberClient) {
		log.info("Redis clients not initialized, nothing to stop.");
		return;
	}
	...
	try {
		...
		log.info("✅ Redis clients disconnected successfully");
	} catch (err) {
